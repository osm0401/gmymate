...
const COMPOSITION_METRICS = [
  { key: "muscleMass", label: "골격근량", unit: "kg", color: "var(--app-accent)" },
...
function renderCompositionCard(logs, metric) {
  ...
  const titleId = `inbody-trend-title-${metric.key}`;
  const descId = `inbody-trend-desc-${metric.key}`;
  const accessibleTitle = `${metric.label} 추이 그래프`;
  const accessibleDesc = `${formatDateKey(first.date)}부터 ${formatDateKey(latest.date)}까지. ` +
  ...
  return `
    <figure class="inbody-metric">
      ...
      <svg viewBox="0 0 ${geometry.width} ${geometry.height}" class="inbody-sparkline" preserveAspectRatio="none" role="img" aria-labelledby="${titleId} ${descId}">
        <title id="${titleId}">${escapeHtml(accessibleTitle)}</title>
        <desc id="${descId}">${escapeHtml(accessibleDesc)}</desc>
...
// 형식뿐 아니라 실제 달력 날짜(윤년, 월별 일수 등)까지 검증한다.
