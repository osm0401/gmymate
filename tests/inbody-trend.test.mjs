...
test("selectTrendPoints: zero is a valid value, not treated as missing", () => {
  const logs = [{ date: "2026-01-01", bodyFat: 0 }];
  assert.deepEqual(selectTrendPoints(logs, "bodyFat"), [{ date: "2026-01-01", value: 0 }]);
});
...
test("selectTrendPoints: invalid dates (bad format or nonexistent calendar day) are excluded", () => {
  const logs = [
    ...
    { date: "2026-02-30", muscleMass: 2 },
    { date: "not-a-date", muscleMass: 3 },
    { date: null, muscleMass: 4 },
    { date: "", muscleMass: 5 },
    { date: "2026-02-28", muscleMass: 6 }
  ];
  assert.deepEqual(selectTrendPoints(logs, "muscleMass"), [{ date: "2026-02-28", value: 6 }]);
});

test("selectTrendPoints: null/undefined log entries are skipped", () => {
  const logs = [null, undefined, { date: "2026-01-01", muscleMass: 30 }];
  assert.deepEqual(selectTrendPoints(logs, "muscleMass"), [{ date: "2026-01-01", value: 30 }]);
});

test("selectTrendPoints: non-array logs input returns empty", () => {
  assert.deepEqual(selectTrendPoints(null, "muscleMass"), []);
  assert.deepEqual(selectTrendPoints(undefined, "muscleMass"), []);
});
...
test("buildTrendGeometry: empty points produce no coords", () => {
  ...
  assert.equal(geometry.min, null);
  assert.equal(geometry.max, null);
});
...
test("buildTrendGeometry: x spacing reflects real day gaps (1:9 ratio for Jul 1/2/11)", () => {
  const points = [
  ...
  const geometry = buildTrendGeometry(points);
  const [p1, p2, p3] = geometry.coords;
  const gapA = p2.x - p1.x;
  const gapB = p3.x - p2.x;
  assert.ok(Math.abs(gapB / gapA - 9) < 0.01, `expected ~9:1 gap ratio, got ${gapB / gapA}`);
});
...
test("selectTrendPoints: date-key math is timezone independent (no UTC-12/UTC+14 shift)", () => {
  const originalTZ = process.env.TZ;
  for (const tz of ["Etc/GMT+12", "Etc/GMT-14"]) {
    process.env.TZ = tz;
    const logs = [
      { date: "2026-01-01", muscleMass: 1 },
      { date: "2026-01-31", muscleMass: 2 }
    ];
    const points = selectTrendPoints(logs, "muscleMass");
    assert.deepEqual(points.map((point) => point.date), ["2026-01-01", "2026-01-31"]);
    assert.equal(formatDateKey("2026-01-01"), "1월 1일");
  }
